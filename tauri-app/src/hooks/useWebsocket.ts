...
import {User, UserWithCursor} from "../redux/user.ts";

//消息类型，下面的代码我用any类型，不建议用any，大家可以自己声明Message类型，为了方便我暂时用any 不用Message

type Message = {
    type: "join" | "broadcast" | "leave";
    data: UserWithCursor | User | string;
};
...
const useWebSocket = (
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            clearTimeout(reconnectTimerRef.current as NodeJS.Timeout);
        };
    }, []);
